---
title: "How Authentication Works in ReservaFácil"
...
<Steps>
  <Step title="Register or Log In">
    The client sends a `POST` request to `/api/auth/register` (with `nombre`, `email`, `password`) or `/api/auth/login` (with `email`, `password`). The API validates the credentials — checking the database for the user record, verifying `activo: true`, and comparing the submitted password against the stored bcrypt hash.
  </Step>
...
- `signToken` — Creates and signs a new JWT from a `JWTPayload`. Sets the algorithm to `HS256` and the expiry to `7d`. Called by the login and register route handlers immediately after validating credentials.
...
The `token` cookie is set with the following attributes in both `/api/auth/login` and `/api/auth/register`:
